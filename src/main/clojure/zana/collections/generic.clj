...
(ns ^{:author "wahpenayo at gmail dot com" 
      ...
      :doc "Generic versions of clojure functions.
            ...
            <dt>foo!</dt>
            <dd>arg is mutable and (may be) modified, return value may be void/nil.</dd>
            <dl>!foo</dl>
  ...
  (:import [java.util ArrayList Arrays Collection Collections HashMap 
            ...
            RandomAccess Set]
           [java.util.concurrent Executors Future ThreadLocalRandom]
...
;; for custom iteration, implement Iterable and Iterator for the data structure.
;;------------------------------------------------------------------------------
(defn has-next? 
  ...
  (and i (.hasNext i)))
;;------------------------------------------------------------------------------
;; Avoid confusion with clojure.core/next (which is a terrible choice of name
;; for an alternative to rest...)
(defn next-item 
...
(defn get
  ...
  ([things k default]
    ...
    (cond (instance? Map things) (.getOrDefault ^Map things k default)
          (instance? Multimap things) (get (.asMap ^Multimap things) k default)
          :else (clojure.core/get things k default)))
  ([things k]
    (assert (not (nil? things)))
    (cond (instance? Multiset things) (.count ^Multiset things k)
          :else (get things k nil))))
;;------------------------------------------------------------------------------
...
(defmethod iterator Iterator  [^Iterator things] things)
(defmethod iterator Iterable [^Iterable things] (.iterator things))
(defmethod iterator nil [_] (Collections/emptyIterator))
(defmethod iterator Map [^Map things] (iterator (.entrySet things)))
(prefer-method iterator Map Iterable)
(defmethod iterator Multimap [^Multimap things] (iterator (.asMap things)))
(defmethod iterator Table [^Table things] (iterator (.cellSet things)))

;; TODO: iterators that return unboxed double and long primitives (and String?)
(let [aclass (class (boolean-array 0))]
  (defmethod iterator aclass [^booleans things] (iterator (Booleans/asList things))))
(let [aclass (class (byte-array 0))] 
  (defmethod iterator aclass [^bytes things] (iterator (Bytes/asList things))))
(let [aclass (class (char-array 0))]
  (defmethod iterator aclass [^chars things] (iterator (Chars/asList things))))
(let [aclass (class (double-array 0))]
  (defmethod iterator aclass [^doubles things] (iterator (Doubles/asList things))))
(let [aclass (class (float-array 0))]
  (defmethod iterator aclass [^floats things] (iterator (Floats/asList things))))
(let [aclass (class (long-array 0))]
  (defmethod iterator aclass [^longs things] (iterator (Longs/asList things))))
(let [aclass (class (short-array 0))]
  (defmethod iterator aclass [^shorts things] (iterator (Shorts/asList things))))
(let [aclass (class (object-array 0))]
  (defmethod iterator aclass [^objects things] (iterator (Arrays/asList things))))
;;------------------------------------------------------------------------------
...
(defn first 
  ...
  (cond (nil? things) nil
        (or (seq? things) (coll? things) 
            ...
            (list? things) (map? things)) (clojure.core/first things) 
        (instance? List things) (when (< 0 (.size ^List things)) 
                                  (.get ^List things 0))
        (instance? String things) (when (< 0 (.length ^String things))
                                    (.charAt ^String things 0))
        (cc/double-array? things) (when (< 0 (alength ^doubles things))
                                    (aget ^doubles things 0))
        (cc/object-array? things) (when (< 0 (alength ^objects things))
                                    (aget ^objects things 0))
        (cc/long-array? things) (when (< 0 (alength ^longs things))
                                  (aget ^longs things 0))
        (cc/int-array? things) (when (< 0 (alength ^ints things))
                                 (aget ^ints things 0))
        (cc/float-array? things) (when (< 0 (alength ^floats things))
                                   (aget ^floats things 0))
        (cc/char-array? things) (when (< 0 (alength ^chars things))
                                  (aget ^chars things 0))
        (cc/byte-array? things) (when (< 0 (alength ^bytes things))
                                  (aget ^bytes things 0))
        (cc/short-array? things) (when (< 0 (alength ^shorts things))
                                   (aget ^shorts things 0))
        (instance? Multimap things) (first (.asMap ^Multimap things))
        :else  (let [it (iterator things)] 
                 (when (.hasNext it) (.next it)))))
;;------------------------------------------------------------------------------
(defn second
  "Return the second element of <code>things</code> in the natural ordering
   of its elements."
  [things] 
  (cond (nil? things) nil
        (or (seq? things) (coll? things) 
            (counted? things) (sequential? things) 
            (list? things) (map? things)) (clojure.core/second things) 
        (instance? List things) (when (< 1 (.size ^List things)) 
                                  (.get ^List things 1))
        (instance? String things) (when (< 1 (.length ^String things))
                                    (.charAt ^String things 1))
        (cc/double-array? things) (when (< 1 (alength ^doubles things))
                                    (aget ^doubles things 1))
        (cc/object-array? things) (when (< 1 (alength ^objects things))
                                    (aget ^objects things 1))
        (cc/long-array? things) (when (< 1 (alength ^longs things))
                                  (aget ^longs things 1))
        (cc/int-array? things) (when (< 1 (alength ^ints things))
                                 (aget ^ints things 1))
        (cc/float-array? things) (when (< 1 (alength ^floats things))
                                   (aget ^floats things 1))
        (cc/char-array? things) (when (< 1 (alength ^chars things))
                                  (aget ^chars things 1))
        (cc/byte-array? things) (when (< 1 (alength ^bytes things))
                                  (aget ^bytes things 1))
        (cc/short-array? things) (when (< 1 (alength ^shorts things))
                                   (aget ^shorts things 1))
        (instance? Multimap things) (second (.asMap ^Multimap things))
        :else  (let [it (iterator things)]
                 (when (.hasNext it) 
                   (.next it)
                   (when (.hasNext it) 
...
;;------------------------------------------------------------------------------
(defn last
  "Return the last element of <code>things</code> in the natural ordering
   of its elements."
  [things] 
  (cond (nil? things) nil
        (or (seq? things) (coll? things) 
            (counted? things) (sequential? things) 
            (list? things) (map? things)) (clojure.core/last things) 
        (instance? List things) (let [n (- (.size ^List things) 1)] 
                                  (when (<= 0 n) (.get ^List things n)))
        (instance? String things) (let [n (- (.length ^String things) 1)] 
                                    (when (<= 0 n) (.charAt ^String things n)))
        (cc/double-array? things) (let [n (- (alength ^doubles things) 1)] 
                                    (when (<= 0 n) (aget ^doubles things n)))
        (cc/object-array? things) (let [n (- (alength ^objects things) 1)] 
                                    (when (<= 0 n) (aget ^objects things n)))
        (cc/long-array? things) (let [n (- (alength ^longs things) 1)] 
                                  (when (<= 0 n) (aget ^longs things n)))
        (cc/int-array? things) (let [n (- (alength ^ints things) 1)] 
                                 (when (<= 0 n) (aget ^ints things n)))
        (cc/float-array? things) (let [n (- (alength ^floats things) 1)] 
                                   (when (<= 0 n) (aget ^floats things n)))
        (cc/char-array? things) (let [n (- (alength ^chars things) 1)] 
                                  (when (<= 0 n) (aget ^chars things n)))
        (cc/byte-array? things) (let [n (- (alength ^bytes things) 1)] 
                                  (when (<= 0 n) (aget ^bytes things n)))
        (cc/short-array? things) (let [n (- (alength ^shorts things) 1)] 
                                   (when (<= 0 n) (aget ^shorts things n)))
        (instance? Multimap things) (last (.asMap ^Multimap things))
        :else  (let [it (iterator things)] 
                 (loop [v nil]
                   (if-not (.hasNext it) 
                     v
...
(defmethod generic-count Iterator [^Iterator things] (long (Iterators/size things)))
(defmethod generic-count Iterable [^Iterable things] (long (Iterables/size things)))
(defmethod generic-count Collection [^Collection things] (long (.size things)))
(prefer-method generic-count Collection Iterable)
(defmethod generic-count Map [^Map things] (long (.size things)))
(prefer-method generic-count Map Iterable)
(defmethod generic-count Multimap [^Multimap things] (generic-count (.asMap things)))
(defmethod generic-count nil [_] (long 0))
(defmethod generic-count String [^String things] (long (.length things)))

(let [aclass (class (boolean-array 0))]
  (defmethod generic-count aclass [^booleans things] (long (alength things))))
(let [aclass (class (byte-array 0))]
  (defmethod generic-count aclass [^bytes things] (long (alength things))))
(let [aclass (class (char-array 0))]
  (defmethod generic-count aclass [^chars things] (long (alength things))))
(let [aclass (class (double-array 0))]
  (defmethod generic-count aclass [^doubles things] (long (alength things))))
(let [aclass (class (float-array 0))]
  (defmethod generic-count aclass [^floats things] (long (alength things))))
(let [aclass (class (long-array 0))]
  (defmethod generic-count aclass [^longs things] (long (alength things))))
(let [aclass (class (short-array 0))]
  (defmethod generic-count aclass [^shorts things] (long (alength things))))
(let [aclass (class (object-array 0))]
  (defmethod generic-count aclass [^objects things] (long (alength things))))
...
#_(defmethod empty? IPersistentCollection [^IPersistentCollection x] 
...
#_(defmethod empty? Sequential [^Sequential x] (clojure.core/empty? x))

(let [aclass (class (boolean-array 0))]
  (defmethod empty? aclass [^booleans x] (>= 0 (long (alength x)))))
(let [aclass (class (byte-array 0))]
  (defmethod empty? aclass [^bytes x] (>= 0 (long (alength x)))))
(let [aclass (class (char-array 0))]
  (defmethod empty? aclass [^chars x] (>= 0 (long (alength x)))))
(let [aclass (class (double-array 0))]
  (defmethod empty? aclass [^doubles x] (>= 0 (long (alength x)))))
(let [aclass (class (float-array 0))]
  (defmethod empty? aclass [^floats x] (>= 0 (long (alength x)))))
(let [aclass (class (int-array 0))]
  (defmethod empty? aclass [^ints x] (>= 0 (long (alength x)))))
(let [aclass (class (long-array 0))]
  (defmethod empty? aclass [^longs x] (>= 0 (long (alength x)))))
(let [aclass (class (short-array 0))]
  (defmethod empty? aclass [^shorts x] (>= 0 (long (alength x)))))
(let [aclass (class (object-array 0))]
  (defmethod empty? aclass [^objects x] (>= 0 (long (alength x)))))
;;------------------------------------------------------------------------------
(defn every? 
  "Does <code>f</code> return a truthy value for every element of things?"
  [f things]
...
;;------------------------------------------------------------------------------
(defmulti builder   
  "Return a mutable object that has add, addAll, etc. methods for adding
   content, and a build method that returns a corresponding immutable object."
...
(defmulti add!   
  {:arglists '([builder-or-prototype thing])
   :doc "Take a mutable container and add some <code>thing</code> to it."}
...
(defmethod add! java.util.Collection [^java.util.Collection things thing]
  (.add things thing)
  things)
;;------------------------------------------------------------------------------
(defmulti add-all!   
...
;;------------------------------------------------------------------------------
;; filter: data structure -> similar data structure of smaller size
;; TODO: (filter f) -> transducer?
;; TODO: (filter-into class-builder-or-prototype f things ...)?
;;------------------------------------------------------------------------------
(defmulti ^Iterable filter 
  "Select the elements of some data structure where (f thing) is truthy, 
   usually returning a similar structure."
  {:arglists '([f things])}
...
;;------------------------------------------------------------------------------
;; split-with: data structure -> 2 similar data structures of smaller size
...
(defmulti ^java.util.List !split-with
  "Return list containing 2 new mutable 'collections':
   ...
   Like running [[filter]] twice, once with <code>f</code> and once with 
   <code>(fn [x] (not (f x)))</code>."
  {:arglists '([f things])}
  (fn !split-with-dispatch ([f things] [(class f) (class things)])))

(defmethod !split-with [IFn nil] [^IFn f _] [(ArrayList.) (ArrayList.)])
...
(defmethod !split-with [IFn RandomAccess] [^IFn f ^RandomAccess things]
...
;;------------------------------------------------------------------------------
(defn map-to-objects 
  "Like [[map]] but with output in an array with element type <code>type</code>."
...
;;       and (map f things0 ...) -? (map-into things0 f things0 ...)
;; TODO: use arglist of f in map-dispatch, to determine if mapping over
;; key, value or entry, ...
;;------------------------------------------------------------------------------
(defmulti map 
  "Apply a function to each 'element' of some data structure, returning a
   structure similar to the first argument."
...
(defmethod map [IFn IPersistentMap] [^IFn f ^IPersistentMap things]
...
(defmethod map 
  ...
  [^IFn f ^Collection things0 ^Collection things1 ^Collection things2]
  (let [a (ArrayList. (min (count things0) (count things1) (count things2)))
        i0 (iterator things0)
        i1 (iterator things1)
        i2 (iterator things2)]
    (while (and (.hasNext i0) (.hasNext i1) (.hasNext i2))
...
(defn nmap-doubles
  ...
  (^doubles [^long n ^clojure.lang.IFn$OD f things0 things1]
    (let [pool (Executors/newFixedThreadPool (int n))
          tasks (map (fn [x0 x1] (callable f x0 x1)) things0 things1)
...
(defn- nmap-collect-results ^Iterable [^long n ^java.util.Collection tasks]
  (let [pool (Executors/newFixedThreadPool (int n))
...
(defmethod nmap 
  ...
  (nmap-collect-results 
    (long n) (map (fn [x0 x1 x2] (callable f x0 x1 x2)) i0 i1 i2)))

(defmethod nmap 
  [IFn Object Object Object] 
  [n ^IFn f ^Object things0 ^Object things1 ^Object things2]
  (nmap n f (iterator things0) (iterator things1) (iterator things2)))
;;------------------------------------------------------------------------------
(defn pmap
...
;;------------------------------------------------------------------------------
(defn nmapc
  "[[nmap]] but purely for side effects; always returns <code>nil</code>."
  ([^long n f things]
    (assert (ifn? f) (print-str "Not a function:" f))
    (let [pool (Executors/newFixedThreadPool (int n))
      ...
      (try
        (doseq [^Future future (.invokeAll pool tasks)] (.get future))
        (finally (.shutdown pool))))
    nil)
...
(defn pmapc
  "[[pmap]] but purely for side effects; always returns <code>nil</code>."
  ([f things0 things1]
    (assert (ifn? f) (print-str "Not a function:" f))
    (nmapc (.availableProcessors (Runtime/getRuntime)) f things0 things1))
  ([f things]
    (assert (ifn? f) (print-str "Not a function:" f))
    (nmapc (.availableProcessors (Runtime/getRuntime)) f things)))
;;------------------------------------------------------------------------------
;; mapcat and friends
;;------------------------------------------------------------------------------
#_(defn concat 
...
;;------------------------------------------------------------------------------
(defn mapcat 
  "More general eager version of <code>clojure.core/mapcat</code>."
  ^java.util.List [f things]
  (assert (ifn? f) (print-str "Not a function:" f))
  (when things
    (let [l (ArrayList.)
...
(defn pmapcat
  ...
  ([f things]
    (assert (ifn? f) (print-str "Not a function:" f))
    (nmapcat (.availableProcessors (Runtime/getRuntime)) f things)))
;;------------------------------------------------------------------------------
;; collection IO
;;------------------------------------------------------------------------------
...
(defmethod clojure.core/print-method 
  ...
  [^com.google.common.collect.RegularImmutableList things ^java.io.Writer w]
  (.write w "[")
  (let [it (iterator things)]
    (when (.hasNext it) (clojure.core/print-method (.next it) w))
    (mapc #(do (.write w " ") (clojure.core/print-method % w)) it))
...
#_(defmulti sort-by 
   ...
   If <code>comparator</code> is not supplied, then the range of <code>z</code> 
   over <code>data</code> must contain only mutually comparable objects."
    {:arglists '([z data] [z comparatorS data])}
    (fn sort-by-dispatch ([& args] (mapv class args))))

;; TODO: methods that extract z values to array of primitives, and do a parallel
;; sort of the array and the list.
...
(defmulti shuffle!
  ...
  "Destructively and randomly permute an ordered 'collection' 
   <code>things</code> using the pseudo-random number generator <code>prng>."
  
  {:arglists '([things] [things ^java.util.Random prng])}
  
  (fn shuffle!-dispatch 
    ([things] [(class things)])
    ([things ^java.util.Random prng] [(class things) (class prng)])))
...
;;------------------------------------------------------------------------------
;; TODO: with a generic copy function, could replace this by a simple function

(defmulti shuffle
  
  "Return a similar ordered 'collection' which contains a random permutation of 
   the elements of <code>things</code> using the pseudo-random number generator 
   <code>prng>."
