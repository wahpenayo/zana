...
(test/deftest empty-datum
  (let [[e0 e1 e2] (setup/empties)
    ...
    (test/is (= "#zana.test.defs.data.empty/Empty {}" 
                (.toString e0)))
...
(test/deftest typical
  (let [ts0 (setup/typicals)
    ...
    #_(test/is 
      (= 
        "#zana.test.defs.data.typical/Typical {
         :n 17 :x 3.141592653589793 :string \"Galileo\" 
         :p {:tf true :b 0 :sh 1 :i 2 :l 3 :f 4.0 :d 5.0 :c \\a} 
         :ymd #object[java.time.LocalDate 0x281f23f2 \"1564-02-15\"] 
         :dt #object[java.time.LocalDateTime 0x87abc48 \"2015-02-20T10:15:30\"]}"
        (.toString (first ts0))))
    (test/is (== 2 (count ts1)))
